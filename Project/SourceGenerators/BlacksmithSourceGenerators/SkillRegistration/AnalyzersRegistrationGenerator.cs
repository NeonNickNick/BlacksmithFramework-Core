...
namespace BlacksmithSourceGenerators.SkillRegistration
{
    [Generator]
    public class AnalyzersRegistrationGenerator : IIncrementalGenerator
    {
        ...
        private static AnalyzerPackageInfo? GetPackageInfo(GeneratorSyntaxContext ctx)
        {
            ...
            foreach (var method in methods)
            {
                ...
                var isAnalyzerAttr = method.GetAttributes().FirstOrDefault(a =>
                    a.AttributeClass?.ToDisplayString() == "BlacksmithCore.Infrastructure.Attributes.Analyzer.IsAnalyzer");

                var isAnalyzerAliasAttr = method.GetAttributes().FirstOrDefault(a =>
                    a.AttributeClass?.ToDisplayString() == "BlacksmithCore.Infrastructure.Attributes.Analyzer.IsAnalyzerAlias");
        ...
        private static INamedTypeSymbol? FindSkillPackageBase(INamedTypeSymbol type)
        {
            ...
            while (current != null)
            {
                if (current.Name == "SkillPackageBase" &&
                    ...
                    current.ContainingNamespace.ToDisplayString() == "BlacksmithCore.Infrastructure.Models.Profession")
                    return current;
        ...
        private static PackageKind DeterminePackageKind(INamedTypeSymbol type)
        {
            ...
            while (current != null)
            {
                ...
                if (fullName == "BlacksmithCore.Infrastructure.Profession.Models.SkillPackageDefinition")
                    return PackageKind.SkillPackageDefinition;
                if (fullName == "BlacksmithCore.Infrastructure.Profession.Models.SkillPackageModifier")
        ...
        private static string[] GetExpectedParameterTypeNames(AnalyzerTypeKind analyzerType)
        {
            return analyzerType switch
            {
                ...
                {
                    "BlacksmithCore.Infrastructure.Models.Player.Community",
                    "BlacksmithCore.Infrastructure.Models.Player.Community",
                    "BlacksmithCore.Infrastructure.Models.AnalyzableDatas.IAnalyzableData"
                },
                AnalyzerTypeKind.Defense => new[]
                {
                    "BlacksmithCore.Infrastructure.Models.Player.Community",
                    "BlacksmithCore.Infrastructure.Models.Player.Community",
                    "BlacksmithCore.Infrastructure.Models.AnalyzableDatas.DefenseEntity",
                    "BlacksmithCore.Infrastructure.Models.AnalyzableDatas.AttackAnalyzableData"
                },
                ...
                AnalyzerTypeKind.Preprocess => new[]
                {
                    "BlacksmithCore.Infrastructure.Models.AnalyzableDatas.IAnalyzableData"
                },
        ...
        private static string GetRegistryMemberName(AnalyzerTypeKind analyzerType)
        {
            return analyzerType switch
            {
                ...
                AnalyzerTypeKind.Preprocess => "Preprocess",
        ...
        private static string GeneratePartialClass(AnalyzerPackageInfo info)
        {
            ...
            foreach (var method in info.AnalyzerMethods)
            {
                ...
                sb.AppendLine($"        global::BlacksmithCore.Infrastructure.SkillSystem.Analyzer.AnalyzerRegistry.{memberName}.Regist(\"{method.MethodName}\", {method.MethodName});");
            }
            foreach (var alias in info.ResolvedAliases)
            {
                var memberName = GetRegistryMemberName(alias.AnalyzerType);
                sb.AppendLine($"        global::BlacksmithCore.Infrastructure.SkillSystem.Analyzer.AnalyzerRegistry.{memberName}.Regist(\"{alias.AliasName}\", {alias.TargetQualifiedReference}.{alias.TargetMethodName});");
            }
            sb.AppendLine("    }");
            sb.AppendLine("}");
            return sb.ToString();
