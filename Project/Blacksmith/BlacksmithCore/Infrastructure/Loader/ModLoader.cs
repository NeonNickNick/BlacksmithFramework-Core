...
namespace BlacksmithCore.Infrastructure.Loader
{
    public static class ModLoader
    {
        ...
        private static void LoadProfessions()
        {
            ...
            {
                if (p is SkillPackageDefinition plugin)
                {
                    ProfessionRegistry.RegistSkillPackageName(plugin.GetType().Name);
            ...
            foreach (var p in ModProfessionPlugins)
            {
                if (p is SkillPackageModifier plugin)
                {
                    var metaData = plugin.GetType().GetCustomAttribute<IsSkillPackageModifier>();
                    if (metaData == null)
                    {
                        continue;
                    }
                    ProfessionRegistry.RegistSkillPackageModifier(metaData.TargetName, plugin);
                    // 元数据直接覆盖
                    ProfessionRegistry.CollectSkillMetadata(plugin);
                    plugin.RegistAnalyzers();
                }
