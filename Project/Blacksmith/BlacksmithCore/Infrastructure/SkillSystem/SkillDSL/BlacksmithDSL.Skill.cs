...
namespace BlacksmithCore.Infrastructure.SkillSystem.SkillDSL
{
    ...
    public static partial class BlacksmithDSL
    {
        ...
        public class SourceFile : IDSLSourceFile
        {
            ...
            public SourceFile Exclude(IReadOnlySet<SentenceType> exclusiveTypes)
            {
                int n = _sentences.Count;
                for (int i = n - 1; i >= 0; i--)
                {
                    if (exclusiveTypes.Contains(_sentences[i].SentenceType))
                    {
            ...
            public EffectFile WriteEffect(
                ...
                string analyzerKey = nameof(StandardAnalyzers.DefaultEffect),
                int delayRounds = 0,
                Func<bool>? ifUndo = null)
            {
                ifUndo ??= DefaultIfUndo;
            ...
            public SourceFile WriteFree(
                ...
                SentenceType sentenceType = SentenceType.Free,
                Func<bool>? ifUndo = null)
            {
                ifUndo ??= DefaultIfUndo;
                _sentences.Add(new(source =>
                {
                    ...
                    action(source);
                }, sentenceType, StructureType.Main));
                ...
                Func<bool>? ifUndo = null)
            {
                return WriteFree(
                    ...
                    sentenceType: SentenceType.ResourceUse,
                    ifUndo: ifUndo);
            }
            public SourceFile UseResource(
                Func<float> need,
            ...
            public SourceFile LoseHP(int loss, Func<bool>? ifUndo = null)
            {
                return WriteFree(
                    ...
                    sentenceType: SentenceType.HPLose,
                    ifUndo: ifUndo);
            }
            ...
            public SourceFile TakeMark(string markName, out Lazy<int> layerNum)
            {
                ...
                return WriteFree(source =>
                {
                    result = source.Focus.TakeMark(markName);
                    isExecuted = true;
                });
            }
            public SourceFile TakeMark(IReadOnlySet<string> markNames, out Lazy<IReadOnlyDictionary<string, int>> layerNums)
            {
                ...
                return WriteFree(source =>
                {
                    var marks = source.Focus.Get<Effect>().Marks;
                    foreach (var markName in markNames)
                    {
                        result[markName] = source.Focus.TakeMark(markName);
                    }
                    isExecuted = true;
                });
            }
            ...
            public SourceFile AddSkillPackageDefinition<TSkillPackageDefinition>(bool isExclusive = true)
                where TSkillPackageDefinition : SkillPackageDefinition, new()
            {
                return WriteFree(source =>
                {
                    var skill = source.Focus.Get<Skill>();
                    skill.AddPackage(new(new TSkillPackageDefinition()));
                    if (isExclusive)
                    {
                        foreach (var name in ProfessionRegistry.ProfessionSkillNames)
                        {
                    ...
                    {
                        skill.RemoveSkill(nameof(Common), nameof(TSkillPackageDefinition).ToLower());
                    }
                });
            }
            public SourceFile AddSkillPackageDefinition<TSkillPackageDefinition>(IReadOnlySet<string> exclusionSet)
                where TSkillPackageDefinition : SkillPackageDefinition, new()
            {
                return WriteFree(source =>
                {
