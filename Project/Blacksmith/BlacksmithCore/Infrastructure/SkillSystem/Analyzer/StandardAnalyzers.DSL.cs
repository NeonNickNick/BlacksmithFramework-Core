...
namespace BlacksmithCore.Infrastructure.SkillSystem
{
    public partial class StandardAnalyzers : SkillPackageDefinition
    {
        ...
        [IsAnalyzer(AnalyzerType.DSL)]
        public static void DefaultEffect(Community player, Community enemy, IAnalyzableData analyzableData)
        {
            var effectData = (EffectAnalyzableData)analyzableData;
            if (effectData.EffectEntity.TargetType == EffectTargetType.Instance.Self())
            {
                player.Focus.Get<Effect>().Add(effectData.EffectEntity);
            }
            else
            {
                enemy.Focus.Get<Effect>().Add(effectData.EffectEntity);
            }
        }
    }
}
