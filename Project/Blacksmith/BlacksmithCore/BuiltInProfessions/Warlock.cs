...
namespace BlacksmithCore.BuiltInProfessions
{
    ...
    public partial class Warlock : SkillPackageDefinition
    {
        ...
        private static bool MuteCheck(ISkillCheckContext sc) => true;
        ...
        private static IDSLSourceFile Mute(ISkillExecuteContext sc)
        {
            Pen pen = sf => sf
               .WriteEffect(
                new()
                {
                    AnalyzerKey = nameof(MuteEffectAnalyzer),
                    Clock = new(),
                    TargetType = EffectTargetType.Instance.Enemy(),
                    Type = EffectType.Instance.AfterTransport()
                });
            return DSL.CreateBy(pen);
        }
